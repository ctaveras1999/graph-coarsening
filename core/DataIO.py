...
from GraphUtils import ensure_tensor_bulk
...
class GraphSampler(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        weights = self.data[idx][1]
        num_nodes = self.data[idx][2]
        ...
        if len(self.data[idx]) > 3:
            label = torch.LongTensor([self.data[idx][3]])
...
def get_data(dataset):
    if   dataset == "aves":
        path =  "../data/aves-weaver-social/aves-weaver-social.edges"
    elif   dataset == "aves-wildbird":
        path =  "../data/aves-wildbird-network/aves-wildbird-network.edges"
    elif dataset == "ant-trophallaxis":
        path =  "../data/insecta-ant-trophallaxis-colony1/insecta-ant-trophallaxis-colony1.edges"
    elif dataset == "enron-employees":
        path =  "../data/ia-enron-employees/ia-enron-employees.edges"
    elif dataset == "enron-dynamic":
        path =  "../data/ia-enron-email-dynamic/ia-enron-email-dynamic.edges"
    elif dataset == "reptilia":
        path =  "../data/reptilia-tortoise-network-mc/reptilia-tortoise-network-mc.edges"
    elif dataset == "raccoon":
        path =  "../data/mammalia-raccoon-proximity/mammalia-raccoon-proximity.edges"
    elif dataset == "ant-colony":
        path = "../data/insecta-ant-colony1/insecta-ant-colony1.edges"
    elif dataset == "hypertext2009":
        path = "../data/ia-contacts_hypertext2009/ia-contacts_hypertext2009.edges"
    elif dataset == "hospital-proximity":
        path = "../data/ia-hospital-ward-proximity-attr/ia-hospital-ward-proximity-attr.edges"
    elif dataset == "primary-school":
        path = "../data/ia-primary-school-proximity/ia-primary-school-proximity.edges"
    elif dataset == "digg":
        path = "../data/ia-digg-reply/ia-digg-reply.edges"
    return path

def get_graphs(dataset, num_blocks=50, self_loops=True):
    path = get_data(dataset)
    if dataset in ["hypertext2009", "hospital-proximity", "primary-school"]:
        df = pandas.read_csv(path, sep=',', header=None, on_bad_lines='skip')
        if dataset == "hospital-proximity":
    ...
    graphs = {}
    keys = sorted(set(data[:,-1]), reverse=False)
    ...
    for i, x in enumerate(data):
        ...
        if t not in graphs:
            graphs[t] = []
        graphs[t].append([u, v, w])
    
    N = min(num_blocks, len(keys))
    width = len(keys)//N
    keys_part = [keys[width*i:width*(i+1)] for i in range(N)]
    
    aggregated_graphs = []

    for keys in keys_part:
        G = nx.Graph()
        edges = set()
        nodes = {}
        curr_node = -1
        for t in keys:
            for (u, v, w) in graphs[t]:
                if (u == v) and (not self_loops):
                    continue
                hat_nodes = [-1, -1]
                for i, x in enumerate([u, v]):
                    if x not in nodes:
                        hat_nodes[i] = curr_node + 1
                        curr_node += 1
                        nodes[x] = hat_nodes[i]
                    if x in nodes:
                        hat_nodes[i] = nodes[x]
                u_hat, v_hat = sorted(hat_nodes)
                edges.add((u_hat, v_hat, w))
        for (u,v,w) in edges:
            G.add_edge(u,v,weight=w)
        aggregated_graphs.append(G)

    return aggregated_graphs, graphs

# removed graph labels from process
def process(graphs, probs=None, feats=None): 
    if isinstance(graphs[0], MeasureNetwork):
       graph_data = graphs

    else:
        if probs is None:
            probs = [None] * len(graphs)
        elif feats is None:
            feats = [None] * len(graphs)
        graph_data = [MeasureNetwork(x,y,z) for (x, y, z) in zip(graphs, probs, feats)]
        
    data_sampler = StructuralDataSampler(graph_data)
    return graph_data, data_sampler
